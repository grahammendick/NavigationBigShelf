...
<div class="sortAndFilter">
    ...
    <asp:ListView ID="SortList" runat="server" ItemType="BigShelf.Controllers.SortViewModel" SelectMethod="GetSortOptions" OnCallingDataMethods="Page_CallingDataMethods">
        ...
        <ItemTemplate>
            <li>
                <nav:NavigationHyperLink ID="sortLink" runat="server" ToData='<%# new NavigationData(){{ "sort" , Item.Sort }, { "sortAscending" , Item.Ascending }, { "page", "" }} %>' Text='<%#: Item.Text %>' Enabled='<%# Item.Enabled %>' CssClass='<%# Item.CssClass %>' Direction="Refresh" IncludeCurrentData="true" />
            </li>
        </ItemTemplate>
    </asp:ListView>
