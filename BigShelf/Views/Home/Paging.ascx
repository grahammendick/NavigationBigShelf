...
<asp:ListView ID="PagingList" runat="server" ItemType="BigShelf.Controllers.PagingViewModel" SelectMethod="GetPages" OnCallingDataMethods="Page_CallingDataMethods">
    ...
    <ItemTemplate>
        <li>
            <nav:NavigationHyperLink ID="pageLink" runat="server" ToData='<%# new NavigationData(){{ "page" , Item.Index }} %>' Text='<%#: Item.GroupText %>' Direction="Refresh" IncludeCurrentData="true" DisableInert="true" InertCssClass="selected" />
        </li>
    </ItemTemplate>
    <EmptyDataTemplate>
        <strong>None</strong>
